...
resource "aws_lb_target_group" "https" {
  ...
  health_check {
    # Probe the plain HTTP listener so a TCP probe does not create a partial
    # TLS handshake on Virtuoso's HTTPS listener.
    port                = 8890
    protocol            = "HTTP"
    path                = "/"
    matcher             = "200-399"
...
resource "aws_lb_target_group" "sql" {
  ...
  health_check {
    # Secure SQL has no HTTP response. Reuse the HTTP listener for health
    # checks instead of opening and closing an incomplete TLS connection.
    port                = 8890
    protocol            = "HTTP"
    path                = "/"
    matcher             = "200-399"
