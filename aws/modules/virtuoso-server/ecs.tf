...
resource "aws_ecs_task_definition" "restore" {
  ...
  container_definitions = jsonencode([{
    ...
    command = [
      "/bin/bash",
      "-lc",
      "set -euo pipefail; cd ${local.virtuoso_database_path}; first_backup=$(find backup -maxdepth 1 -type f -name 'virtuoso_#*.bp' -printf '%f\\n' | sort | head -n 1); if [ -z $first_backup ]; then first_backup=$(find backup -maxdepth 1 -type f -name 'virtuoso_*.bp' -printf '%f\\n' | sort | head -n 1); fi; if [ -z $first_backup ]; then echo No Virtuoso backup files found in backup; exit 1; fi; backup_prefix=$(printf '%s' $first_backup | sed -E 's/[0-9]+\\.bp$//'); for backup_file in backup/$backup_prefix*.bp; do if [ -e $backup_file ]; then cp $backup_file .; fi; done; restore_dir=restore-pre-$(date -u +%Y%m%dT%H%M%SZ); mkdir -p $restore_dir; for f in virtuoso.db virtuoso.log virtuoso.trx virtuoso-temp.db virtuoso.pxa virtuoso.lck; do if [ -e $f ]; then mv $f $restore_dir/; fi; done; server=$(command -v virtuoso-t || command -v virtuoso-iodbc-t || find / -name virtuoso-t -o -name virtuoso-iodbc-t 2>/dev/null | head -1); if [ -z $server ]; then echo Virtuoso server binary not found; exit 1; fi; if ! $server +foreground +configfile virtuoso.ini +restore-backup $backup_prefix; then exit 1; fi; rm -f $backup_prefix*.bp"
    ]
