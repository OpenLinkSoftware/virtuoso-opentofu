...
HAS_ANY_BP=0
HAS_ONLINE_CHAIN=0
if find "\$BACKUP_DIR" -maxdepth 1 -type f -name 'virtuoso_*.bp' | grep -q .; then
  HAS_ANY_BP=1
fi
if find "\$BACKUP_DIR" -maxdepth 1 -type f -name 'virtuoso_#*.bp' | grep -q .; then
  HAS_ONLINE_CHAIN=1
fi
if [ "\$HAS_ANY_BP" = "1" ] && [ "\$HAS_ONLINE_CHAIN" = "0" ]; then
  ARCHIVE_DIR="\$BACKUP_DIR/legacy-\$(date -u +%Y%m%d%H%M%S)"
  mkdir -p "\$ARCHIVE_DIR"
  find "\$BACKUP_DIR" -maxdepth 1 -type f -name 'virtuoso_*.bp' -exec mv {} "\$ARCHIVE_DIR"/ \;
fi
if [ "\$HAS_ONLINE_CHAIN" = "0" ]; then
  BACKUP_INIT_SQL="backup_context_clear();
checkpoint;"
fi
BACKUP_SQL="\$BACKUP_INIT_SQL
delete from DB.DBA.SYS_BACKUP_DIRS;
insert into DB.DBA.SYS_BACKUP_DIRS (bd_id, bd_dir) values (1, '\$BACKUP_DIR');
backup_online('\$BACKUP_PREFIX', 10000, 0, vector('\$BACKUP_DIR'));
"
BACKUP_LOG="/tmp/virtuoso-online-backup.out"
printf "%s\n" "\$BACKUP_SQL" | "\$ISQL" 1112 dba "\$DBA_PASSWORD" -E 2>&1 | tee "\$BACKUP_LOG"
if grep -Eq '^\\*\\*\\* Error|^Error ' "\$BACKUP_LOG"; then
  echo "Virtuoso online backup failed" >&2
  exit 1
fi
if echo "\$BACKUP_INIT_SQL" | grep -q backup_context_clear; then
  echo "Created initial full online backup"
else
  echo "Created incremental online backup"
fi
