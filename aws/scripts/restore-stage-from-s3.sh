...
if [[ ! "$BACKUP_URI" =~ ^s3://[A-Za-z0-9][A-Za-z0-9.-]{1,61}[A-Za-z0-9](/[^[:space:]\"\\]*)?$ ]]; then
  echo "Backup URI must be a valid s3://bucket[/prefix] URI without whitespace or quotes" >&2
  exit 1
fi

OVERRIDES="$(jq -nc --arg source "$BACKUP_URI" --arg destination "$DB_PATH/backup/" \
  '{containerOverrides: [{name: "backup-sync", command: ["s3", "sync", $source, $destination]}]}')"
