...
insert replacing DB.DBA.SYS_SCHEDULED_EVENT (SE_NAME, SE_START, SE_INTERVAL, SE_SQL) values (DB.DBA.BACKUP_SCHED_NAME(), dateadd('minute', 5, now()), $INTERVAL_MINUTES, 'DB.DBA.BACKUP_MAKE(''virtuoso_'', 10000, 0)');
...
printf "%s\n" "$SQL" | "$ISQL" 1111 dba "$DBA_PASSWORD"
EOF
