...
5. Restricting HTTP/HTTPS access to trusted CIDRs
6. Optionally exposing secure SQL only to trusted CIDRs
...
virtuoso_image_tag = "latest" # Pin a tested release tag or immutable digest in production.
...
Configure daily online backup generation inside Virtuoso. This configures `DB.DBA.SYS_BACKUP_DIRS` and schedules `backup_online()` so Conductor can show the backup directory and schedule state:

```bash
./scripts/setup-scheduled-backup.sh 1440
```

Verify that the generated `virtuoso_*.bp` backup chain has been copied to the configured S3 URI:

```bash
aws s3 ls "$(tofu output -raw backup_s3_uri)" \
  --recursive \
  --human-readable \
  --summarize
```
...
```bash
# Run from the aws/ deployment directory so OpenTofu can read this deployment's state.
REGION="$(tofu output -raw region)"
CLUSTER="$(tofu output -raw ecs_cluster_name)"
SERVICE="$(tofu output -raw ecs_service_name)"

# Get the running task ARN. Resource names depend on the project_name selected at deployment.
...
2. **Restart the service:**
   ```bash
   # Stop
   aws ecs update-service --region "$(tofu output -raw region)" --cluster "$(tofu output -raw ecs_cluster_name)" --service "$(tofu output -raw ecs_service_name)" --desired-count 0

   # Wait for task to stop
   aws ecs list-tasks --region "$(tofu output -raw region)" --cluster "$(tofu output -raw ecs_cluster_name)" --service-name "$(tofu output -raw ecs_service_name)"

   # Start
   aws ecs update-service --region "$(tofu output -raw region)" --cluster "$(tofu output -raw ecs_cluster_name)" --service "$(tofu output -raw ecs_service_name)" --desired-count 1
   ```
...
The default DBA secret recovery window is seven days. This protects against accidental deletion, but means that a same-name redeployment must wait for recovery-window expiry or use a different `project_name`. Set `dba_secret_recovery_window_in_days = 0` only for disposable test deployments where immediate deletion is required.
